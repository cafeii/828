...
# 适配自 third_party/GatedDeltaNet-2/lit_gpt/model.py：
# 1) flash_attn / fused_rotary_embedding / mixers.gdn2 改为惰性import（CPU环境可import本模块）；
# 2) Block 按 config.mixer / config.mixer_per_layer 分派 RNN mixer 或 CausalSelfAttention。
...
except ImportError:  # 本地CPU环境无flash_attn：attention层forward在GPU环境才可用
...
class Block(nn.Module):
    def __init__(self, config: Config, layer_idx: int) -> None:
        ...
        self.use_rnn = (
            (config.mixer != "attn") and config.mixer_per_layer > 0
            and layer_idx % config.mixer_per_layer == 0
        )
        if self.use_rnn:
