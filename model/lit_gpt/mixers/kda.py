# KDA 骨架 token mixer。模块结构对齐 mixers/gdn2.py，门参数化/初始化对齐
# fla.layers.kda 的 KimiDeltaAttention（f_proj 低秩 + A_log/dt_bias，b_proj 无 bias）。
#
# 递归：S_t = (I - β_t k k^T) Diag(exp(g_t)) S_{t-1} + β_t k v^T
#   g_t 为 channel-wise decay（log域 g:[B,T,H,d_k]），β_t 为 per-head 标量。
# GQA/LSR 化的关键约束：β 与 decay 必须组级（若逐头则组内各头 state 不同，P 无法提出），q 保持逐头。
# KDA ≡ GDN2 取 b=β·1（k维广播）、w=β·1（v维广播）（GDN2 kernel docstring 原话），
# naive 模式据此复用 naive_gdn2_recurrence；chunk/fused_recurrent 走 fla 的 kda kernel
# （g:[B,T,H,K] 预计算 log-decay、beta:[B,T,H]，use_gate_in_kernel=False，见 fla.ops.kda 文档）。
# LSR（use_lsr=True）：v 为组级潜向量 c，写入 β k c^T，出口乘静态 P 还原（同 gdn2.py 策略2）。
#
# 与 fla 原版 KDA 的有意差异：输出门用 SiLU 门控 RMSNorm（原版为 sigmoid 门），
# 与本项目 GDN/GDN2 骨架保持一致，便于跨骨架对照归因。
...
class KimiDeltaAttention(nn.Module):
    """KDA token mixer，统一支持 MHA / GQA / GQA+expand_v / GQA+LSR 形态。

    形状约定（G=num_groups, I=num_heads//G, d_k=head_dim,
    d_v=head_dim*expand_v, d_c=lsr_latent_dim）：
      q: [B,T,H,d_k] 逐头
      k/g: [B,T,G,d_k] 组级（g为channel-wise log-decay）
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gk_dim = self.num_groups * self.head_k_dim  # k/g组级
        ...
        # channel-wise遗忘门（组级，log-decay），低秩瓶颈与KDA原版/gdn2.py一致
        self.f_proj = nn.Sequential(
            nn.Linear(hidden_size, self.head_v_dim, bias=False),
            nn.Linear(self.head_v_dim, self.gk_dim, bias=False),
        )
        # 擦除写入门β（组级标量），KDA原版无bias
        self.b_proj = nn.Linear(hidden_size, self.num_groups, bias=False)

        # 遗忘门参数（组级），初始化方式与KDA原版一致
        self.A_log = nn.Parameter(torch.log(torch.empty(self.num_groups, dtype=torch.float32).uniform_(1, 16)))
        self.A_log._no_weight_decay = True
        dt = torch.exp(
            torch.rand(self.gk_dim, dtype=torch.float32) * (math.log(0.1) - math.log(0.001)) + math.log(0.001)
        ).clamp(min=1e-4)
        ...
        # 输出路径：逐头SiLU门控RMSNorm + 输出投影（g_proj低秩与KDA原版一致）
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 组级channel-wise log遗忘门，fp32保证下游cumsum数值稳定
        g = (
        ...
        k, g = (rearrange(x, "... (g d) -> ... g d", d=self.head_k_dim) for x in (k, g))
        v = rearrange(v, "... (g d) -> ... g d", d=self.latent_dim)

        # 门控统计（组级张量，repeat 之前）：遗忘门 g(channel-wise log-decay) / 擦除写入门 β
        ...
        if self.heads_per_group > 1:
            k, g, v = (repeat(x, "... g d -> ... (g i) d", i=self.heads_per_group) for x in (k, g, v))
            b = repeat(b, "... g -> ... (g i)", i=self.heads_per_group)
        ...
        if mode == "chunk":
            from ..kernels import get_chunk_kda

            o, recurrent_state = get_chunk_kda()(
            ...
            from ..kernels import get_fused_recurrent_kda

            o, recurrent_state = get_fused_recurrent_kda()(
                q=q, k=k, v=v, g=g, beta=b,
                initial_state=recurrent_state,
                output_final_state=output_state,
                use_qk_l2norm_in_kernel=True,
            ...
            # KDA ≡ GDN2：b=β·1（k维）、w=β·1（v维），g为channel-wise直接对应
