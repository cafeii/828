...
def dense_transition(q, k, v, g, beta, lamb, *, initial_state):
...
def test_fp64_formula_output_state_and_all_input_gradients():
    xs = [x.requires_grad_() for x in inputs()]
    ref = [x.detach().clone().requires_grad_() for x in xs]
    actual = qdelta_reference(*xs[:6], initial_state=xs[6])
    expected = dense_transition(*ref[:6], initial_state=ref[6])
...
def test_qdelta_model_preserves_gdn_backbone_and_learns_lambda():
    ...
    models = []
    for name in ("gdn_recall_tiny", "qdelta_recall_tiny"):
        ...
        models.append(model)
    gdn, qdelta = models
    qdelta_parameters = dict(qdelta.named_parameters())
    for name, parameter in gdn.named_parameters():
        torch.testing.assert_close(parameter, qdelta_parameters[name], rtol=0, atol=0)
    lambda_parameters = [p for n, p in qdelta.named_parameters() if ".lambda_proj." in n]
    assert sum(p.numel() for p in lambda_parameters) == 2 * 128 * 2
    for block in qdelta.transformer.h:
        block.attn.mode = "naive"
    logits = qdelta(torch.randint(0, 256, (2, 13)))
    F.cross_entropy(logits.flatten(0, 1), torch.randint(0, 256, (26,))).backward()
    assert all(p.grad is not None and p.grad.isfinite().all() and p.grad.abs().sum() > 0 for p in lambda_parameters)


@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA parity requires an allocated GPU")
def test_cuda_output_state_and_all_input_gradients():
    ...
    actual = qdelta_rule(*gpu[:6], initial_state=gpu[6], output_final_state=True)
    expected = qdelta_reference(*ref[:6], initial_state=ref[6])
    for value, target in zip(actual, expected):
