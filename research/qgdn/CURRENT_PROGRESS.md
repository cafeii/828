...
seed 42/1234 的六条严格配对 340M/10BT 复现实验全部完成并回收；结合历史 seed 3407，
三方法九条均跑满 19073 step / 9,999,745,024 prediction tokens。最终 PPL 均值 ± 样本SD：
GDN `14.837867 ± 0.030363`，QGDN Recall→Delta `14.835654 ± 0.029418`，
论文正号 Q-Delta `14.814083 ± 0.018532`。Q-Delta 相对两个基线均 3/3 胜出，平均
配对 PPL 分别降低 0.16017% / 0.14530%；收益较小，不宣称统计显著。

当前任务已收尾，`qgdn-seed` 监控已暂停；最新依据见文末“三 seed 10BT 最终结论与监控结束”。
冻结代码、数据与科学配置未改动，不重复提交已完成实验。QGDN 使用虚拟 2T，
`QGDN_USE_PHYSICAL_T=False`；物理 T 优化继续按用户要求暂存。
...
step-12000 validation 继续保持同方向，但领先幅度收窄：Recall→Delta loss/PPL 为
`2.761304 / 15.82045`，相对 GDN `2.762260 / 15.83558` 低 `0.000956` loss、
`0.096%` PPL；Parallel 与 Delta→Recall 的 PPL 为 `15.83187 / 15.85808`。
Recall→Delta 现已通过 step `12661`，最新 beta mean/std 为
...
step-16000 validation 仍维持微弱领先：Recall→Delta loss/PPL 为
`2.713727 / 15.08540`，相对 GDN `2.714649 / 15.09932` 低 `0.000922` loss、
`0.092%` PPL，相对 Parallel `15.09035` 低 `0.033%`，相对 Delta→Recall
`15.11940` 低 `0.225%`。作业现至 step `16661`，近 20 点 loss `2.67928`，beta
mean/std `0.29524 / 0.17125`，gamma mean/std/饱和率
`0.39832 / 0.30204 / 4.81%`，稳态吞吐约 `310.9k token/s`。八个共同验证点方向一致，
但相对 GDN 的收益仍不足千分之一，最终结论需等终点。

Recall→Delta Slurm `37183` 已成功跑完并回收：Slurm `COMPLETED / 0:0`、
`run.exitcode=0`、summary `completed`、preflight JUnit `6/6`，终点 step `19073`、
prediction tokens `9,999,745,024`。训练计算/完整墙钟为 `8.893 / 9.019 h`，
有效训练/墙钟吞吐为 `312.33k / 307.99k token/s`，峰值显存
`77.0250 GB/GPU`。最终 validation loss/PPL 为 `2.696375 / 14.82588`，相对
GDN `14.84041` 低 `0.098%`，相对 Parallel `14.82890` 低 `0.020%`，相对
Delta→Recall `14.86030` 低 `0.232%`。末步 beta mean/std 为
`0.29414 / 0.17227`，gamma mean/std/饱和率为
`0.39617 / 0.30182 / 4.76%`，loss、grad norm 和门控统计全部有限。这使
Recall→Delta 在四路终点中排名第一，但相对 GDN 仍只是约千分之一的小信号。
本地/远端 preflight、metrics、summary 的 SHA-256 均一致，约 `4.14 GB` checkpoint
按回收规则保留在远端。
...
后续监控至 Recall→Delta step `7391`、Parallel step `7321`，全部训练记录的 gamma
仍逐点精确为 `1.0 / 0.0 / 100%`，loss 与 grad norm 有限，最新 beta mean/std 分别为
`0.25549 / 0.15378` 和 `0.25233 / 0.15271`。近 20 点 loss 为
`2.82538 / 2.82573`，稳态吞吐约 `316.4k / 313.2k token/s`；无异常日志，checkpoint
持续更新。

第四个共同 step-8000 validation 继续确认负向信号：固定 gamma=1 的
Recall→Delta/Parallel loss/PPL 为 `2.842080 / 17.15141`、
`2.841971 / 17.14953`，比各自 beta-style 对照高 `0.417% / 0.283%` PPL，
比 GDN 高 `0.304% / 0.293%`。两路现至 step `8751 / 8661`，全部记录的 gamma
仍严格为 `1.0 / 0.0 / 100%`；beta mean/std 为
`0.25606 / 0.15504` 与 `0.25424 / 0.15416`，近 20 点 loss 为
`2.79677 / 2.79376`。连续四个共同验证点都不支持固定强 recall。

第五个共同 step-10000 validation 延续同一结论。固定 gamma=1 的
Recall→Delta/Parallel loss/PPL 为 `2.799789 / 16.44118`、
`2.799699 / 16.43970`，比各自 beta-style 对照高 `0.399% / 0.318%` PPL，
也比 GDN 高 `0.255% / 0.246%`。作业现至 step `10191 / 10091`，beta
mean/std 为 `0.25772 / 0.15607` 与 `0.25611 / 0.15540`，gamma 仍在全部记录中
...
第七个共同 step-14000 validation 继续复现：固定 gamma=1 的
Recall→Delta/Parallel loss/PPL 为 `2.737784 / 15.45271`、
`2.737444 / 15.44745`，比各自 beta-style 对照高 `0.379% / 0.289%` PPL，
也比 GDN 高 `0.277% / 0.243%`。两路现至 step `14141 / 14001`，
beta mean/std 为 `0.26212 / 0.15805` 与 `0.25812 / 0.15711`，gamma 仍全程严格为
`1.0 / 0.0 / 100%`；近 20 点 loss 为 `2.71083 / 2.71541`。无 OOM、非有限值或
配置偏离，连续七个共同验证点都不支持固定强 recall。

固定 gamma=1 的两个作业已成功完成并回收。Recall→Delta / Parallel 的
Slurm `37379 / 37380` 均为 `COMPLETED / 0:0`，`run.exitcode=0`，preflight
JUnit `10/10`，终点为 step `19073` 和 `9,999,745,024` prediction tokens。最终
validation loss/PPL 为 `2.699919 / 14.87853` 与 `2.699549 / 14.87302`，比各自
beta-style 对照高 `0.355% / 0.297%` PPL，也比 GDN 高
`0.257% / 0.220%`。末步 beta mean/std 为 `0.25857 / 0.15874` 与
`0.25554 / 0.15780`，gamma 在全部训练记录中始终严格为
`1.0 / 0.0 / 100%`。训练计算/墙钟为 `8.731/8.849 h` 与
`8.815/8.944 h`，有效训练/墙钟吞吐为 `318.15k/313.90k` 与
`315.12k/310.57k token/s`，峰值显存 `75.6754 / 75.7184 GB/GPU`。固定强
recall 在终点也被否决。

## 可训练 gamma∼U(0.85, 0.95) 消融结果
...
第三个共同 step-6000 validation 已完成。高 gamma Recall→Delta 的 loss/PPL 为
`2.895069 / 18.08475`，比同顺序 beta-style 高 `0.281%` PPL、比固定 gamma=1 低
`0.166%`；高 gamma Parallel 为 `2.893995 / 18.06533`，比 beta-style 高
`0.023%`、比固定 1 低 `0.245%`。相对 GDN，两者分别高 `0.083%` 和低 `0.024%`，
都属于很小的差异。到共同 step `6451`，高 gamma Recall→Delta/Parallel 的近 20 点
loss 为 `2.86154 / 2.86057`，beta mean/std 为
`0.28145 / 0.16091` 与 `0.28003 / 0.16094`，gamma mean/std/饱和率为
`0.52453 / 0.31461 / 11.28%` 与 `0.52535 / 0.31201 / 10.72%`。因此 gate 极化仍在
缓慢退潮，数值稳定；三个验证点仍支持“可训练明显优于固定 1”，但不支持高 gamma
初始化稳定优于 beta-style。

到共同 step `7781`，高 gamma Recall→Delta/Parallel 的近 20 点 loss 为
`2.82595 / 2.82504`，beta mean/std 为 `0.28443 / 0.16262` 与
`0.28250 / 0.16274`，gamma mean/std/饱和率为
`0.51811 / 0.31376 / 10.70%` 与 `0.51741 / 0.31204 / 10.31%`。早期极化继续缓慢
回落，全部数值有限；两路接近 step-8000 validation，保持原配置等待下一共同节点。

第四个共同 step-8000 validation 已完成。高 gamma Recall→Delta 的 loss/PPL 为
`2.840848 / 17.13029`，比同顺序 beta-style 高 `0.294%` PPL，但比固定 gamma=1
低 `0.123%`；高 gamma Parallel 为 `2.839491 / 17.10706`，比 beta-style 高
`0.035%`，比固定 1 低 `0.248%`。两路相对 GDN 分别高 `0.181% / 0.045%`，仍无
稳定收益。现至 step `9261 / 9191`，beta mean/std 为
`0.28931 / 0.16458` 与 `0.28734 / 0.16400`，gamma mean/std/饱和率为
`0.51014 / 0.31363 / 10.09%` 与 `0.50839 / 0.31167 / 9.82%`；饱和率从早期约
19% 继续下降，全部 loss 和梯度有限。四个验证点共同支持“可训练优于固定 1”，但不支持
`U(0.85,0.95)` 初始化优于 beta-style。

第五个共同 step-10000 validation 仍无高初值收益。高 gamma Recall→Delta 的
loss/PPL 为 `2.798340 / 16.41737`，比同顺序 beta-style 高 `0.253%` PPL、比固定
gamma=1 低 `0.145%`；高 gamma Parallel 为 `2.797475 / 16.40318`，比 beta-style
高 `0.095%`、比固定 1 低 `0.222%`。相对 GDN 分别高 `0.110% / 0.024%`。
两路现至 step `10521 / 10441`，beta mean/std 为
`0.28998 / 0.16540` 与 `0.28834 / 0.16574`，gamma mean/std/饱和率为
`0.50631 / 0.31321 / 9.94%` 与 `0.50592 / 0.31201 / 9.70%`；所有指标有限，
checkpoint 和日志持续更新。五个验证点一致表明可训练高 gamma 优于固定 1，但没有优于
beta-style 初始化。

第六个共同 step-12000 validation 仍是同一排序。高 gamma
Recall→Delta/Parallel loss/PPL 为 `2.763985 / 15.86294`、
`2.763101 / 15.84892`；相对同顺序 beta-style 高 `0.269% / 0.108%` PPL，
相对固定 gamma=1 低 `0.089% / 0.182%`，相对 GDN 高
`0.173% / 0.084%`。现至 step `13161 / 13061`，beta mean/std 为
`0.29208 / 0.16637` 与 `0.29047 / 0.16630`，gamma mean/std/饱和率为
`0.50085 / 0.31353 / 9.70%` 与 `0.49780 / 0.31186 / 9.31%`；近 20 点 loss 为
`2.72337 / 2.72699`，loss、梯度、日志和 checkpoint 继续健康。六个共同节点
均支持“可训练比固定 1 好，但高初值没有超过 beta-style”。

可训练高 gamma 的两个作业也已成功完成并回收。Recall→Delta / Parallel 的
Slurm `37413 / 37414` 均为 `COMPLETED / 0:0`，`run.exitcode=0`，preflight
JUnit `13/13`。最终 validation loss/PPL 为 `2.698479 / 14.85712` 与
`2.697799 / 14.84702`：比各自 beta-style 对照高 `0.211% / 0.122%` PPL，但比固定
gamma=1 低 `0.144% / 0.175%`，相对 GDN 仍高 `0.113% / 0.045%`。末步 beta
mean/std 为 `0.28953 / 0.16810` 与 `0.28842 / 0.16793`，gamma mean/std/饱和率为
`0.48994 / 0.31519 / 9.56%` 与 `0.48626 / 0.31390 / 9.15%`，所有 loss 和梯度
有限。训练计算/墙钟为 `8.841/8.967 h` 与 `8.920/9.045 h`，有效训练/墙钟
吞吐为 `314.18k/309.76k` 与 `311.40k/307.11k token/s`，峰值显存
...
首个严格配对的 step-2000 validation 已完成。正号/负号 loss/PPL 分别为
`3.139583 / 23.09423` 与 `3.141828 / 23.14613`：负号比正号高 `0.2247%` PPL；
正号比 GDN 低 `0.2236%`、比 beta-style Recall→Delta 低 `0.1060%`，负号则与 GDN
几乎相同（高 `0.00068%`）、比 Recall→Delta 高 `0.1185%`。在对齐 step 1991，
最近 20 个训练记录的 loss 为 `3.14627 / 3.14912`；beta mean/std 为
`0.27591/0.16412` 与 `0.28256/0.16624`，lambda mean/std 为
`0.34656/0.25138` 与 `0.36224/0.25899`。两条作业无非有限数值，收缩区间违规率均为
`0`，step-2000 checkpoint 完整。单个早期验证点暂只支持“正号领先负号”，最终效果仍须
继续跑满 10BT 确认。

第二个严格配对的 step-4000 validation 也已完成。正号/负号 loss/PPL 为
`2.971961 / 19.53018` 与 `2.973236 / 19.55509`，负号高 `0.1275%` PPL；正号
相对 GDN、beta-style Recall→Delta 和 beta-style Parallel 分别低
`0.2451% / 0.0777% / 0.3237%` PPL，负号相对 GDN 低 `0.1179%`，但比
Recall→Delta 高 `0.0497%`。对齐 step 4101 的近 20 个训练记录 loss 为
`2.94383 / 2.94526`；正号/负号 alpha mean/std 为 `0.68946/0.34256` 与
`0.69179/0.34076`，beta mean/std 为 `0.27563/0.16139` 与 `0.28078/0.16408`，
lambda mean/std 为 `0.36102/0.25407` 与 `0.38306/0.26195`。两条作业的 loss/梯度
全部有限，收缩区间违规率仍为 `0`，step-4000 checkpoint 完整，无 CUDA/NCCL/OOM
错误。两个共同节点均支持论文正号，但领先幅度从 `0.2247%` 收窄到 `0.1275%`，因此
仍按早期小信号处理并继续跑满 10BT。

step-6000 和 step-8000 的两个后续共同 validation 也已完成。正号/负号 PPL 分别为
`18.02264 / 18.03731` 与 `17.06370 / 17.08113`，负号分别高 `0.0814% / 0.1021%`。
正号在这两个节点相对 GDN 低 `0.2608% / 0.2086%` PPL，相对 beta-style
Recall→Delta 低 `0.0632% / 0.0962%`；负号相对 GDN 低 `0.1796% / 0.1066%`，
但与 Recall→Delta 基本持平（高 `0.0181% / 0.0058%`）。对齐 step 8081 的近 20 点
训练 loss 为 `2.81421 / 2.81474`；正号/负号 alpha mean/std 为
`0.68570/0.34395` 与 `0.68828/0.34192`，beta mean/std 为
`0.28596/0.16531` 与 `0.29080/0.16882`，lambda mean/std 为
`0.37202/0.25555` 与 `0.39324/0.26239`。当前正号/负号分别到达 step `10111 / 8081`，
正号已额外完成 step-10000 validation（PPL `16.36178`），等待负号到达相同节点后再配对解读。
...
step-12000 和 step-14000 的第六、七个共同 validation 继续支持论文正号。两节点
正号/负号 PPL 分别为 `15.80459/15.82089` 与 `15.37581/15.39630`，负号分别高
`0.1031%/0.1333%`。正号相对 GDN 低 `0.1957%/0.2220%`，相对 beta-style
Recall→Delta 低 `0.1002%/0.1202%`；负号相对 GDN 仍低 `0.0928%/0.0890%`，
但相对 Recall→Delta 已基本持平并略高 `0.0028%/0.0129%`。对齐 step 15381 的
近 20 点训练 loss 为 `2.68545/2.68631`，吞吐约 `467.7k/474.8k token/s`；两条
运行在不同节点，不把吞吐差归因于符号。最新正号/负号分别到 step `17411/15421`，
loss、梯度、alpha/beta/lambda/alignment 全部有限，收缩区间违规率均为 `0`，峰值显存
`66.2665/66.2684 GB/GPU`，checkpoint 与日志持续更新且无 CUDA/NCCL/OOM 错误。
正号已完成 step-16000 validation（PPL `15.06649`），等待负号到达该节点后再严格配对。
...
## 两个 seed 的共同验证进展（2026-09-06 00:37 CST）

seed 42 三路已到共同 step 4000 / 2,097,152,000 prediction tokens；seed 1234 三路首次
到共同 step 2000 / 1,048,576,000 prediction tokens。验证均使用相同的 1600 条序列、
6,553,600 scored tokens。各 seed 单独对齐：训练窗口分别为 step 3801–3991 和
1801–1991，每隔 10 step 一个日志点，各取共同验证前最后 20 个完全相同的 step。

| seed | 共同 step | 方法 | validation loss | PPL | 同步训练窗口 loss 均值 |
|---|---:|---|---:|---:|---:|
| 42 | 4000 | GDN | 2.973997917 | 19.57000266 | 2.951653 |
| 42 | 4000 | QGDN Recall→Delta | 2.974556559 | 19.58093834 | 2.952106 |
| 42 | 4000 | Q-Delta 正号 | 2.974112872 | 19.57225245 | 2.952126 |
| 1234 | 2000 | GDN | 3.144988769 | 23.21941470 | 3.134540 |
| 1234 | 2000 | QGDN Recall→Delta | 3.146863037 | 23.26297492 | 3.135978 |
| 1234 | 2000 | Q-Delta 正号 | 3.144971890 | 23.21902279 | 3.134327 |

seed 42 的 Q-Delta 相对 GDN 为 +0.0115% PPL，相对 QGDN 为 -0.0444%；此前 step 2000
相对 GDN 的 +0.1580% 差距已经缩小。seed 1234 的 Q-Delta 相对 GDN 为 -0.0017% PPL，
相对 QGDN 为 -0.1889%。这两个节点的 Q-Delta 均略优于 QGDN，与 GDN 基本持平；
仍不能确认它相对 GDN 的跨 seed 稳定收益。两个 seed 当前预算不同，不混合为多 seed 统计。

| seed / 对齐 step | 方法 | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|---|
| 42 / 3991 | GDN | 0.70312/0.33526 | 0.27137/0.16576 | — | — | — |
| 42 / 3991 | QGDN Recall→Delta | 0.66863/0.36640 | 0.27204/0.16468 | 0.43348/0.31286/7.654% | — | — |
| 42 / 3991 | Q-Delta 正号 | 0.70558/0.33370 | 0.27397/0.16701 | — | 0.37607/0.27093 | 0.27298/0.16824 |
| 1234 / 1991 | GDN | 0.70499/0.32962 | 0.27112/0.15816 | — | — | — |
| 1234 / 1991 | QGDN Recall→Delta | 0.66563/0.36239 | 0.27219/0.16091 | 0.45132/0.30822/8.407% | — | — |
| 1234 / 1991 | Q-Delta 正号 | 0.70281/0.33031 | 0.27231/0.15821 | — | 0.35283/0.25361 | 0.27129/0.16002 |

所有已记录 loss、梯度和门控统计均有限，两条 Q-Delta 全历史收缩违规率仍为 0。
QGDN beta 饱和率未被冻结日志输出，继续标记缺失，不修改运行中的训练代码。

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | GDN | 38983 | 13731 | 861.2k | 56.81 | 13000 |
| 42 | QGDN Recall→Delta | 38984 | 4981 | 314.9k | 77.02 | 4000 |
| 42 | Q-Delta 正号 | 38985 | 7571 | 472.7k | 66.27 | 7000 |
| 1234 | GDN | 38986 | 13491 | 867.5k | 56.82 | 13000 |
| 1234 | QGDN Recall→Delta | 38987 | 2331 | 314.9k | 77.02 | 2000 |
| 1234 | Q-Delta 正号 | 38988 | 2791 | 473.5k | 66.27 | 2000 |

六条均 RUNNING，入口门禁、各 seed 三路共享初始化 hash、实际科学配置、冻结 commit 和
干净 worktree 均核验通过；每个完整作业名仍只有一条活动记录。上表仅用于成本与健康检查，
各作业位于不同节点且训练进度不同。六个已发布 checkpoint 的 ZIP 目录和标量 step 元数据
均可读取；未进行完整 tensor payload CRC 校验。六路日志中的 16 条 IB/RoCE 初始化 warning
计数各自未增长，无实际 NCCL 失败、OOM、非有限值或门禁失败证据。

00:38 已回收六路日志和小型指标；run.exitcode 和最终 summary 仍未生成，回收器仅因
缺 run/summary.json 返回非零，不能标记为终态回收。大权重保留远端。只读 SSH 审计成功，
没有提交、重启或改动训练环境；物理 T 仍关闭。现有 qgdn-seed 半小时监控继续 ACTIVE，
下一次三路效果判断等待 seed 42 的 step 6000 和 seed 1234 的 step 4000。

## seed 42 共同 step 6000 与 launcher 异常复核（2026-09-06 01:20 CST）

seed 42 三路完成共同 step 6000 / 3,145,728,000 prediction tokens validation，
均使用相同的 1600 条序列、6,553,600 scored tokens。同步训练窗口为 step 5801、5811、
…、5991 的 20 个共同日志点。

| 方法 | validation loss | PPL | step 5801–5991 同步训练 loss 均值 |
|---|---:|---:|---:|
| GDN | 2.891743281 | 18.02470435 | 2.875882 |
| QGDN Recall→Delta | 2.891544840 | 18.02112787 | 2.876269 |
| Q-Delta 正号 | 2.891139619 | 18.01382681 | 2.875730 |

Q-Delta 相对 GDN 的 PPL 低 0.06035%，相对 QGDN 低 0.04051%。相对 GDN 从 step 2000
的 +0.1580%、step 4000 的 +0.0115% 转为小幅领先，幅度仍小；不能提前当作稳定多 seed
收益。seed 1234 的 QGDN 到 step 3891，尚未完成新的三路共同 step 4000。

| 方法（共同 step 5991） | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|
| GDN | 0.70342/0.33396 | 0.27778/0.16683 | — | — | — |
| QGDN Recall→Delta | 0.66964/0.36601 | 0.27802/0.16637 | 0.41683/0.30750/6.365% | — | — |
| Q-Delta 正号 | 0.70288/0.33631 | 0.27924/0.16797 | — | 0.37875/0.26898 | 0.27855/0.16950 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step | srun error 行数 |
|---|---|---:|---:|---:|---:|---:|---:|
| 42 | GDN | 38983 | 17921 | 861.3k | 56.81 | 17000 | 52 |
| 42 | QGDN Recall→Delta | 38984 | 6531 | 314.8k | 77.02 | 6000 | 67 |
| 42 | Q-Delta 正号 | 38985 | 9991 | 471.9k | 66.27 | 9000 | 0 |
| 1234 | GDN | 38986 | 17731 | 866.8k | 56.82 | 17000 | 53 |
| 1234 | QGDN Recall→Delta | 38987 | 3891 | 314.7k | 77.02 | 3000 | 54 |
| 1234 | Q-Delta 正号 | 38988 | 5201 | 473.1k | 66.27 | 5000 | 66 |

01:18 审计发现五条作业 stderr 新增 `srun: error:`；第一条均在第 32 行，内容为
`con_close_on_poll_error` / `Connection reset by peer`。后续含 `Insane message length`、
`protocol_version ... not supported`、`Incompatible versions of client and server code`、
`io_init_msg_unpack: unpack error` 等 Slurm 收到消息后的解析/连接诊断。来源和根因尚未确认；
仅凭这些字符串不能断定安装版本确实不匹配。38985 未出现此类错误。

01:18→01:20 间隔超过两分钟复核，六路训练 step 分别由
`17701/6451/9871/17501/3811/5071` 前进至上表进度；五路 stderr 字节数、mtime 和错误行数
均未增长，metrics 写入时间均晚于各自最后 stderr 更新。已记录 loss、梯度、吞吐、显存和
门控统计均有限，两条 Q-Delta 全历史收缩违规率仍为 0。六路 squeue/sacct 均 RUNNING、
无重复作业，run.exitcode 与最终 summary 仍未生成。本轮不重启、不改环境；保留异常记录，
后续同时跟踪 launcher 错误类型/计数、训练进度与最终退出码，不将此次诊断标记为已解决。

冻结 commit、干净 worktree、科学配置、门禁、各 seed 三路初始化 hash 和物理 T 关闭状态
均通过核验。六个 checkpoint ZIP 目录与 step 元数据可读取，未作完整 tensor CRC 校验。
01:19 已回收全部日志与小型指标，回收器仅因缺最终 run/summary.json 返回非零，大权重仍在
...
## 两个共同节点均小幅领先（2026-09-06 02:31 CST）

seed 42 完成三路共同 step 8000 / 4,194,304,000 prediction tokens，seed 1234 完成
共同 step 6000 / 3,145,728,000 prediction tokens。各节点均使用固定 1600 条序列、
6,553,600 scored tokens；训练窗口分别为 step 7801–7991 与 5801–5991，每隔 10 step
一个日志点，各取同一 seed 三路完全相同的最后 20 个日志点。

| seed / 共同 step | 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---|---:|---:|---:|
| 42 / 8000 | GDN | 2.837719362 | 17.07677515 | 2.815353 |
| 42 / 8000 | QGDN Recall→Delta | 2.837956377 | 17.08082309 | 2.815324 |
| 42 / 8000 | Q-Delta 正号 | 2.837041730 | 17.06520730 | 2.814908 |
| 1234 / 6000 | GDN | 2.895346742 | 18.08977283 | 2.872863 |
| 1234 / 6000 | QGDN Recall→Delta | 2.896061667 | 18.10271030 | 2.873259 |
| 1234 / 6000 | Q-Delta 正号 | 2.894777117 | 18.07947138 | 2.872187 |

seed 42 的 Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.06774% / 0.09142%`；seed 1234
分别低 `0.05695% / 0.12837%`。后者从 step 4000 对 GDN 小幅落后转为小幅领先。
两个最新共同节点均有小收益，但幅度很小、两 seed 当前预算不同，仍不混合统计或提前
确认稳定收益；最终配对判断等待其余四路全部跑满。

| seed / gate step | 方法 | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|---|
| 42 / 7991 | GDN | 0.70424/0.33339 | 0.28442/0.16923 | — | — | — |
| 42 / 7991 | QGDN Recall→Delta | 0.67088/0.36588 | 0.28451/0.16932 | 0.41081/0.30511/5.785% | — | — |
| 42 / 7991 | Q-Delta 正号 | 0.70526/0.33454 | 0.28568/0.17057 | — | 0.38164/0.26923 | 0.28503/0.17208 |
| 1234 / 5991 | GDN | 0.69264/0.33650 | 0.27782/0.16258 | — | — | — |
| 1234 / 5991 | QGDN Recall→Delta | 0.65811/0.36466 | 0.28081/0.16403 | 0.41017/0.29461/5.552% | — | — |
| 1234 / 5991 | Q-Delta 正号 | 0.69485/0.33315 | 0.27970/0.16104 | — | 0.36672/0.25616 | 0.27908/0.16358 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step | srun error 行数 |
|---|---|---:|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 9091 | 315.1k | 77.02 | 9000 | 134 |
| 42 | Q-Delta 正号 | 38985 | 13991 | 471.6k | 66.27 | 13000 | 67 |
| 1234 | QGDN Recall→Delta | 38987 | 6461 | 314.8k | 77.02 | 6000 | 134 |
| 1234 | Q-Delta 正号 | 38988 | 9191 | 473.5k | 66.27 | 9000 | 132 |

四条仍 RUNNING，所有已记录 loss、梯度、吞吐、显存及门控数值有限，两条 Q-Delta
全历史收缩违规率均为 0。吞吐为各自近期样本中位数，节点及进度不同，仅用于成本/健康
记录。GDN 38983/38986 的 COMPLETED/0:0、完整预算、终点指标和已回收产物保持不变。
六路冻结 commit、干净 worktree、实际科学配置、入口门禁、初始化 hash 及物理 T 关闭状态
再次核验通过，无重复作业；checkpoint ZIP 目录和 step 元数据可读取，未作完整 tensor CRC。

本轮只有 38985 的 launcher 诊断从 58 增至 67 行，新增九行均属既有协议/消息解析类型，
最后 stderr 更新为 02:01:40；其余五路计数不变。四路训练均继续推进，未见 OOM、
非有限值或退出失败。诊断来源/根因仍未确认，继续保存证据，不重启或修改环境。
02:32 已回收四路更新日志和小型指标，仅缺运行中的最终 summary；两条 GDN 沿用已验证
的终态回收，大权重保留远端。现有半小时监控继续 ACTIVE，下一次等待 seed 42 共同
step 10000 和 seed 1234 共同 step 8000。

## seed 42 共同 step 10000（2026-09-06 03:06 CST）

seed 42 完成三路共同 step 10000 / 5,242,880,000 prediction tokens validation，
均为固定 1600 条序列、6,553,600 scored tokens。同步训练窗口取 step 9801、9811、…、9991
的同 20 个日志点。

| 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---:|---:|---:|
| GDN | 2.795159577 | 16.36524006 | 2.770490 |
| QGDN Recall→Delta | 2.795305854 | 16.36763410 | 2.770054 |
| Q-Delta 正号 | 2.794767811 | 16.35882998 | 2.770084 |

Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.03917% / 0.05379%`，优势较 step 8000
缩小，仍是很小的阶段差异。同步训练窗口中 QGDN 与 Q-Delta 基本持平；继续按冻结预算
跑满，不提前认定跨 seed 稳定收益。seed 1234 的 QGDN 到 step 7721，最新共同验证仍为
step 6000，等待 step 8000。

| 方法（共同 step 9991） | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|
| GDN | 0.70194/0.33414 | 0.28626/0.16946 | — | — | — |
| QGDN Recall→Delta | 0.67011/0.36553 | 0.28776/0.16987 | 0.40418/0.30399/5.582% | — | — |
| Q-Delta 正号 | 0.70259/0.33551 | 0.28848/0.17125 | — | 0.38176/0.26961 | 0.28780/0.17270 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 10361 | 315.3k | 77.02 | 10000 |
| 42 | Q-Delta 正号 | 38985 | 15951 | 472.0k | 66.27 | 15000 |
| 1234 | QGDN Recall→Delta | 38987 | 7721 | 314.9k | 77.02 | 7000 |
| 1234 | Q-Delta 正号 | 38988 | 11161 | 473.2k | 66.27 | 11000 |

四路均 RUNNING，所有已记录数值有限，两条 Q-Delta 全历史收缩违规率仍为 0。各方法
位于不同节点，上表吞吐取最近 20 个日志点中位数，仅用于成本/健康跟踪。两条 GDN 的
COMPLETED/0:0、完整终点预算及已回收结果不变。六路冻结 commit、干净 worktree、实际
科学配置、入口门禁、各 seed 三路初始化 hash、物理 T 关闭状态再次核验通过，无重复作业。
checkpoint ZIP 目录/step 元数据均可读取，未作完整 tensor CRC。

本轮六路 stderr 的字节数、mtime 与 launcher 错误计数均未变化，按 38983–38988 顺序
仍为 `97/134/67/96/134/132`；旧诊断根因仍未确认，保留记录，不标记为已修复。
03:07 已回收四路新日志与小型指标，仅缺运行中的最终 summary，大权重保留远端。
没有重启、重提或改环境。现有半小时监控继续 ACTIVE，等待 seed 42 共同 step 12000
和 seed 1234 共同 step 8000。

## seed 1234 共同 step 8000（2026-09-06 03:40 CST）

seed 1234 完成三路共同 step 8000 / 4,194,304,000 prediction tokens validation，
使用相同的固定 1600 条序列、6,553,600 scored tokens。同步训练窗口为 step 7801、7811、
…、7991 的 20 个相同日志点。

| 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---:|---:|---:|
| GDN | 2.840467677 | 17.12377206 | 2.825722 |
| QGDN Recall→Delta | 2.841153581 | 17.13552135 | 2.826438 |
| Q-Delta 正号 | 2.839448295 | 17.10632528 | 2.824412 |

Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.10189% / 0.17038%`，比 step 6000 的
`0.05695% / 0.12837%` 优势扩大；同步训练窗口 loss 也稍低。收益仍小，最终跨 seed
统计继续等待全部方法跑满。seed 42 的最新三路共同验证仍为 step 10000，Q-Delta
已推进到 step 17861，但尚未到终态，不能提前标记完成。

| 方法（共同 step 7991） | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|
| GDN | 0.69078/0.33810 | 0.28151/0.16604 | — | — | — |
| QGDN Recall→Delta | 0.65735/0.36590 | 0.28518/0.16694 | 0.40347/0.29344/5.189% | — | — |
| Q-Delta 正号 | 0.69421/0.33370 | 0.28513/0.16454 | — | 0.37005/0.25673 | 0.28445/0.16706 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 11591 | 314.8k | 77.02 | 11000 |
| 42 | Q-Delta 正号 | 38985 | 17861 | 471.5k | 66.27 | 17000 |
| 1234 | QGDN Recall→Delta | 38987 | 8951 | 314.7k | 77.02 | 8000 |
| 1234 | Q-Delta 正号 | 38988 | 13071 | 472.8k | 66.27 | 13000 |

四路均 RUNNING、所有已记录数值有限，两条 Q-Delta 全历史收缩违规率为 0。上表吞吐
按各作业最近 20 个日志点中位数，节点/进度不同，仅用于成本和健康跟踪。两条 GDN 的
COMPLETED/0:0、完整预算和已回收终点结果保持不变。六路冻结 commit、干净 worktree、
实际科学配置、入口门禁、各 seed 三路初始化 hash 和物理 T 关闭状态再次核验通过，
无重复作业；checkpoint ZIP 目录/step 元数据可读取，未作完整 tensor CRC。

本轮六路 stderr 字节数、mtime 和 launcher 错误计数均未变化，仍为
`97/134/67/96/134/132`；旧诊断根因未确认，记录继续保留。03:41 已回收四路新日志与
小型指标，仅缺最终 summary，大权重保留远端。没有重启、重提或改环境。
现有半小时监控继续 ACTIVE，等待 seed 42 共同 step 12000、seed 1234 共同 step 10000
及 seed 42 Q-Delta 的完整终态证据。

## seed 42 Q-Delta 完成与两个新共同节点（2026-09-06 04:14 CST）

seed 42 的 Q-Delta（Slurm `38985`）于 04:03:04 CST 完成完整 19073 step /
9,999,745,024 prediction tokens。主作业、batch 和训练 step 均 COMPLETED/0:0，
run.exitcode=0；最终 validation loss `2.694630432`、PPL `14.80004812`，summary、
metrics 与 stdout 的最终验证一致。终点 checkpoint ZIP 目录/step 元数据可读取，
model_final.pt 已写完且无临时 checkpoint；未做 tensor payload 完整 CRC 校验。
04:18 回收器返回 0，必需日志/小型结果齐全，大权重保留远端。

seed 42 / 1234 分别新增三路共同 step 12000 / 10000，训练预算为 6,291,456,000 /
5,242,880,000 prediction tokens。各节点均使用固定 1600 条序列、6,553,600 scored tokens；
同步训练窗口分别取 step 11801–11991 / 9801–9991 的相同 20 个日志点。

| seed / 共同 step | 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---|---:|---:|---:|
| 42 / 12000 | GDN | 2.761179154 | 15.81848438 | 2.739968 |
| 42 / 12000 | QGDN Recall→Delta | 2.761304781 | 15.82047175 | 2.739657 |
| 42 / 12000 | Q-Delta 正号 | 2.760687124 | 15.81070312 | 2.739949 |
| 1234 / 10000 | GDN | 2.798125144 | 16.41384431 | 2.770142 |
| 1234 / 10000 | QGDN Recall→Delta | 2.798336159 | 16.41730824 | 2.769564 |
| 1234 / 10000 | Q-Delta 正号 | 2.796700813 | 16.39048220 | 2.768877 |

Q-Delta 相对 GDN / QGDN 的 PPL 分别低：seed 42 `0.04919% / 0.06175%`，seed 1234
`0.14233% / 0.16340%`。两节点均小幅领先；seed 42 同步训练窗口中 QGDN loss 最低，
seed 1234 则为 Q-Delta 最低。不同 seed 的当前共同预算不同，不混合统计；完整三方法
终点及跨 seed 结论继续等待剩余三条跑满。

| seed / gate step | 方法 | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|---|
| 42 / 11991 | GDN | 0.70023/0.33582 | 0.28976/0.17090 | — | — | — |
| 42 / 11991 | QGDN Recall→Delta | 0.66947/0.36639 | 0.29047/0.17139 | 0.39993/0.30265/5.370% | — | — |
| 42 / 11991 | Q-Delta 正号 | 0.70181/0.33624 | 0.29175/0.17269 | — | 0.38216/0.27015 | 0.29109/0.17415 |
| 1234 / 9991 | GDN | 0.69106/0.33812 | 0.28689/0.16892 | — | — | — |
| 1234 / 9991 | QGDN Recall→Delta | 0.65744/0.36556 | 0.29091/0.16948 | 0.39896/0.29216/4.937% | — | — |
| 1234 / 9991 | Q-Delta 正号 | 0.69166/0.33621 | 0.28943/0.16721 | — | 0.37057/0.25723 | 0.28878/0.16986 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 12811 | 314.7k | 77.02 | 12000 |
| 1234 | QGDN Recall→Delta | 38987 | 10171 | 314.6k | 77.02 | 10000 |
| 1234 | Q-Delta 正号 | 38988 | 14981 | 472.7k | 66.27 | 14000 |

其余三条均 RUNNING，所有已记录数值有限，两条 Q-Delta 全历史收缩违规率为 0。
上表吞吐取各自近期 20 个日志点中位数，节点及进度不同，仅用于成本/健康跟踪。
六路冻结 commit、干净 worktree、实际科学配置、入口门禁、各 seed 三路初始化 hash
和物理 T 关闭状态再次核验通过，无重复作业；checkpoint 目录/step 元数据可读取。

本轮六路 stderr 字节数、mtime 与 launcher 错误计数均未变化，按 38983–38988 顺序仍为
`97/134/67/96/134/132`；旧诊断根因未确认，继续保留。04:18 三条运行中作业的新日志与
小型指标回收完成，仅缺最终 summary。没有重启、重提或改环境；三条已完成作业
38983/38985/38986 均已核验并回收。现有半小时监控继续 ACTIVE，下一次等待 seed 42 /
1234 的共同 step 14000 / 12000 和剩余终态。

## seed 42 共同 step 14000（2026-09-06 04:53 CST）

seed 42 新增三路共同 step 14000 / 7,340,032,000 prediction tokens，固定验证
1600 条序列、6,553,600 scored tokens；同步训练窗口为 step 13801、13811、…、13991
的 20 个相同日志点。

| 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---:|---:|---:|
| GDN | 2.733159420 | 15.38140666 | 2.712391 |
| QGDN Recall→Delta | 2.733742626 | 15.39037980 | 2.712048 |
| Q-Delta 正号 | 2.732987695 | 15.37876551 | 2.712179 |

Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.01717% / 0.07546%`。对 GDN 的优势从
step 12000 的 `0.04919%` 缩小，仍接近持平；同步训练窗口中 QGDN loss 略低。
seed 1234 最新共同验证仍为 step 10000，等待 step 12000；不提前汇总跨 seed 终点结论。

| 方法（共同 step 13991） | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|
| GDN | 0.69878/0.33653 | 0.29059/0.17105 | — | — | — |
| QGDN Recall→Delta | 0.66891/0.36660 | 0.29178/0.17172 | 0.39723/0.30310/5.337% | — | — |
| Q-Delta 正号 | 0.70119/0.33644 | 0.29366/0.17329 | — | 0.38344/0.27113 | 0.29296/0.17467 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 14201 | 314.8k | 77.02 | 14000 |
| 1234 | QGDN Recall→Delta | 38987 | 11571 | 314.8k | 77.02 | 11000 |
| 1234 | Q-Delta 正号 | 38988 | 17141 | 473.2k | 66.27 | 17000 |

三条仍 RUNNING，所有已记录 loss、梯度、吞吐、显存及门控数值有限，两条 Q-Delta
全历史收缩违规率均为 0。吞吐取各自最近 20 个日志点中位数，各作业节点和进度不同。
两条 GDN 与 seed 42 Q-Delta 的完整终态、已回收结果及成本保持不变。六路冻结 commit、
干净 worktree、实际科学配置、入口门禁、各 seed 三路初始化 hash、物理 T 关闭状态
再次核验通过，无重复作业；checkpoint ZIP 目录/step 元数据可读取，未作完整 tensor CRC。

本轮六路 stderr 的字节数、mtime 与 launcher 错误计数均未变化，仍为
`97/134/67/96/134/132`；启动时 NET/IB 告警也未增长。旧诊断根因仍未确认，记录继续
保留。04:53 已回收三条运行中作业的新日志和小型指标，仅缺最终 summary，大权重
保留远端。未重启、重提或改环境；现有半小时监控继续 ACTIVE，等待 seed 42 / 1234
共同 step 16000 / 12000 及剩余终态。

## seed 1234 Q-Delta 完成与共同 step 12000（2026-09-06 05:30 CST）

seed 1234 的 Q-Delta（Slurm `38988`）于 05:28:13 CST 完成完整 19073 step /
9,999,745,024 prediction tokens。05:28 初查时仍在退出阶段；05:30 复核主作业、batch
和训练子任务均 COMPLETED/0:0，run.exitcode=0。最终 validation loss `2.696995281`、
PPL `14.83508941`；summary、metrics 与 stdout 的全部验证节点一致，训练日志也与审计
记录一致。终点 checkpoint ZIP 目录/step 元数据可读取，model_final.pt 已写完且无临时
checkpoint；未作 tensor payload 完整 CRC 校验。05:28 回收器返回 0，必需日志和小型
结果齐全，大权重保留远端。至此两条 GDN 和两条 Q-Delta 均已完成并回收。

seed 1234 新增三路共同 step 12000 / 6,291,456,000 prediction tokens，固定验证
1600 条序列、6,553,600 scored tokens；同步训练窗口为 step 11801、11811、…、11991
的 20 个相同日志点。

| 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---:|---:|---:|
| GDN | 2.764406538 | 15.86961918 | 2.743356 |
| QGDN Recall→Delta | 2.764668591 | 15.87377841 | 2.742953 |
| Q-Delta 正号 | 2.762925853 | 15.84613865 | 2.741739 |

Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.14796% / 0.17412%`，较 step 10000 的
`0.14233% / 0.16340%` 略扩大；同步训练窗口的 loss 也稍低。seed 42 最新共同节点仍为
step 14000。两条 QGDN 尚在训练，三方法最终同预算比较与多 seed 统计继续等待它们跑满。

| 方法（共同 step 11991） | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|
| GDN | 0.68932/0.33908 | 0.28719/0.16958 | — | — | — |
| QGDN Recall→Delta | 0.65673/0.36574 | 0.29175/0.16985 | 0.39447/0.29291/4.922% | — | — |
| Q-Delta 正号 | 0.69071/0.33648 | 0.29047/0.16799 | — | 0.37238/0.25823 | 0.28968/0.17043 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 15541 | 314.9k | 77.02 | 15000 |
| 1234 | QGDN Recall→Delta | 38987 | 12911 | 314.8k | 77.02 | 12000 |

两条 QGDN 均 RUNNING，所有已记录 loss、梯度、吞吐、显存及门控数值有限；两条
已完成 Q-Delta 的全历史收缩违规率均为 0。吞吐取各自最近 20 个日志点中位数，各作业
节点和进度不同。六路冻结 commit、干净 worktree、实际科学配置、入口门禁、各 seed
三路初始化 hash 和物理 T 关闭状态再次核验通过，无重复作业；checkpoint 目录/step
元数据可读取。

本轮六路 stderr 字节数、mtime 与 launcher 错误计数均未变化，仍为
`97/134/67/96/134/132`；启动时 NET/IB 告警也未增长。旧诊断根因仍未确认，继续保留。
05:28 已回收两条 QGDN 的新日志和小型指标，仅缺运行中的最终 summary；没有重启、
重提或改环境。四条终态作业 38983/38985/38986/38988 均已核验并回收，现有半小时
监控继续 ACTIVE，等待 seed 42 / 1234 的共同 step 16000 / 14000 和两条 QGDN 终态。

## 两个 seed 的新共同节点（2026-09-06 06:04 CST）

seed 42 / 1234 分别新增三路共同 step 16000 / 14000，训练预算为 8,388,608,000 /
7,340,032,000 prediction tokens。各节点均使用固定 1600 条序列、6,553,600 scored tokens；
同步训练窗口分别取 step 15801–15991 / 13801–13991 的相同 20 个日志点。

| seed / 共同 step | 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---|---:|---:|---:|
| 42 / 16000 | GDN | 2.712537545 | 15.06746141 | 2.683439 |
| 42 / 16000 | QGDN Recall→Delta | 2.713100241 | 15.07594220 | 2.683522 |
| 42 / 16000 | Q-Delta 正号 | 2.712169294 | 15.06191383 | 2.683417 |
| 1234 / 14000 | GDN | 2.737142313 | 15.44279131 | 2.711468 |
| 1234 / 14000 | QGDN Recall→Delta | 2.737021002 | 15.44091804 | 2.711403 |
| 1234 / 14000 | Q-Delta 正号 | 2.735068439 | 15.41079810 | 2.710294 |

Q-Delta 相对 GDN / QGDN 的 PPL 分别低：seed 42 `0.03682% / 0.09305%`，seed 1234
`0.20717% / 0.19507%`。对 GDN 的优势分别由前一共同节点的 `0.01717% / 0.14796%`
扩大；seed 42 仍接近持平，seed 1234 的收益也仅约 0.2%。两个同步训练窗口中 Q-Delta
loss 均略低。两 seed 当前预算不同，不混合统计；最终三方法配对结论仍待两条 QGDN 跑满。

| seed / gate step | 方法 | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|---|
| 42 / 15991 | GDN | 0.70196/0.33601 | 0.28902/0.17137 | — | — | — |
| 42 / 15991 | QGDN Recall→Delta | 0.67178/0.36693 | 0.28973/0.17223 | 0.39435/0.30275/5.275% | — | — |
| 42 / 15991 | Q-Delta 正号 | 0.70277/0.33772 | 0.29117/0.17361 | — | 0.38495/0.27180 | 0.29054/0.17503 |
| 1234 / 13991 | GDN | 0.68876/0.34019 | 0.28650/0.16983 | — | — | — |
| 1234 / 13991 | QGDN Recall→Delta | 0.65770/0.36582 | 0.29194/0.17043 | 0.39238/0.29312/4.868% | — | — |
| 1234 / 13991 | Q-Delta 正号 | 0.68975/0.33789 | 0.28962/0.16822 | — | 0.37385/0.25938 | 0.28883/0.17066 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 16771 | 314.9k | 77.02 | 16000 |
| 1234 | QGDN Recall→Delta | 38987 | 14131 | 314.7k | 77.02 | 14000 |

两条 QGDN 均 RUNNING，所有已记录 loss、梯度、吞吐、显存及门控数值有限；两条
已完成 Q-Delta 的全历史收缩违规率仍为 0。上表吞吐取各自最近 20 个日志点中位数，
各作业节点与进度不同。四条已完成 GDN/Q-Delta 的终态、回收结果和成本保持不变。
六路冻结 commit、干净 worktree、实际科学配置、入口门禁、各 seed 三路初始化 hash
及物理 T 关闭状态再次核验通过，无重复作业；checkpoint ZIP 目录/step 元数据可读取，
未作完整 tensor CRC。

本轮六路 stderr 字节数、mtime 和 launcher 错误计数均未变化，仍为
`97/134/67/96/134/132`；启动时 NET/IB 告警未增长，旧诊断根因仍未确认，继续保留。
06:05 已回收两条 QGDN 的新日志与小型指标，仅缺运行中的最终 summary，大权重保留
远端。未重启、重提或改环境；现有半小时监控继续 ACTIVE，下一次等待 seed 42 / 1234
共同 step 18000 / 16000 和两条 QGDN 终态。

## seed 42 共同 step 18000（2026-09-06 06:38 CST）

seed 42 新增三路共同 step 18000 / 9,437,184,000 prediction tokens，固定验证
1600 条序列、6,553,600 scored tokens；同步训练窗口为 step 17801、17811、…、17991
的 20 个相同日志点。

| 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---:|---:|---:|
| GDN | 2.699520103 | 14.87259270 | 2.674340 |
| QGDN Recall→Delta | 2.699938781 | 14.87882083 | 2.674116 |
| Q-Delta 正号 | 2.699072918 | 14.86594338 | 2.674288 |

Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.04471% / 0.08655%`。对 GDN 的优势较
step 16000 的 `0.03682%` 略扩大，对 QGDN 则从 `0.09305%` 略缩小，仍是很小差异。
同步训练窗口中 QGDN loss 略低。seed 42 QGDN 尚未到终态；seed 1234 最新共同验证仍为
step 14000，等待 step 16000。最终三方法同预算及多 seed 结论继续等待两条 QGDN 跑满。

| 方法（共同 step 17991） | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|
| GDN | 0.69886/0.33797 | 0.28660/0.17105 | — | — | — |
| QGDN Recall→Delta | 0.66993/0.36741 | 0.28817/0.17214 | 0.39375/0.30404/5.370% | — | — |
| Q-Delta 正号 | 0.70035/0.33871 | 0.29016/0.17369 | — | 0.38501/0.27239 | 0.28944/0.17497 |

| seed | 方法 | Slurm | 已记录 step | 近20点吞吐中位 token/s | 峰值 GB/GPU | checkpoint step |
|---|---|---:|---:|---:|---:|---:|
| 42 | QGDN Recall→Delta | 38984 | 18001 | 315.2k | 77.02 | 18000 |
| 1234 | QGDN Recall→Delta | 38987 | 15371 | 314.8k | 77.02 | 15000 |

两条 QGDN 均 RUNNING，所有已记录 loss、梯度、吞吐、显存及门控数值有限；两条
已完成 Q-Delta 的全历史收缩违规率仍为 0。吞吐取各自近期 20 个日志点中位数，节点与
当前进度不同。四条已完成作业的终态、已回收结果和成本保持不变。六路冻结 commit、
干净 worktree、实际科学配置、入口门禁、各 seed 三路初始化 hash 和物理 T 关闭状态
再次核验通过，无重复作业；checkpoint ZIP 目录/step 元数据可读取，未作完整 tensor CRC。

本轮六路 stderr 字节数、mtime 与 launcher 错误计数均未变化，仍为
`97/134/67/96/134/132`；启动时 NET/IB 告警也未增长，旧诊断根因未确认，继续保留。
06:39 已回收两条 QGDN 的新日志和小型指标，仅缺运行中的最终 summary，大权重保留
远端。没有重启、重提或改环境；现有半小时监控继续 ACTIVE，等待 seed 42 的完整
step 19073 终态及 seed 1234 的共同 step 16000。

## seed 42 三路终点与 seed 1234 共同 step 16000（2026-09-06 07:12 CST）

seed 42 的 QGDN Recall→Delta（Slurm `38984`）于 07:08:54 CST 完成完整 19073 step /
9,999,745,024 prediction tokens。主作业、batch 和训练子任务均 COMPLETED/0:0，
run.exitcode=0；最终 validation loss `2.695462307`、PPL `14.81236503`。summary、
metrics 与 stdout 的全部验证节点一致，完整训练日志与审计记录一致。终点 checkpoint
ZIP 目录/step 元数据可读取，model_final.pt 已写完且无临时 checkpoint；未作 tensor
payload 完整 CRC 校验。07:13 回收器返回 0，必需日志和小型结果齐全，大权重保留远端。
至此 seed 42 三路均完成并回收，本批六条仅剩 seed 1234 的 QGDN。

seed 42 新增共同终点 step 19073 / 9,999,745,024 prediction tokens；seed 1234 新增
共同 step 16000 / 8,388,608,000 prediction tokens。各验证固定 1600 条序列、6,553,600
scored tokens。seed 42 同步训练窗口取 step 18891、18901、…、19071 加终点 19073 的
相同 20 个日志点；seed 1234 取 step 15801–15991 的相同 20 个日志点。

| seed / 共同 step | 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---|---:|---:|---:|
| 42 / 19073 | GDN | 2.695053703 | 14.80631387 | 2.684289 |
| 42 / 19073 | QGDN Recall→Delta | 2.695462307 | 14.81236503 | 2.684074 |
| 42 / 19073 | Q-Delta 正号 | 2.694630432 | 14.80004812 | 2.683803 |
| 1234 / 16000 | GDN | 2.716834583 | 15.13234617 | 2.693173 |
| 1234 / 16000 | QGDN Recall→Delta | 2.716760010 | 15.13121774 | 2.692957 |
| 1234 / 16000 | Q-Delta 正号 | 2.714750751 | 15.10084574 | 2.691764 |

seed 42 最终 Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.04232% / 0.08315%`，是幅度
很小的同预算收益；最终同步训练窗口的 loss 也稍低。seed 1234 在共同 step 16000 分别
低 `0.20817% / 0.20072%`。后者尚未完成三路终点对比，两 seed 当前预算不同，继续等待
最后一条 QGDN 跑满后，再结合 seed 3407 汇总均值、样本标准差、配对差及胜率。

| seed / gate step | 方法 | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|---|
| 42 / 19073 | GDN | 0.69830/0.33863 | 0.28812/0.17109 | — | — | — |
| 42 / 19073 | QGDN Recall→Delta | 0.66905/0.36843 | 0.28928/0.17202 | 0.39247/0.30419/5.357% | — | — |
| 42 / 19073 | Q-Delta 正号 | 0.69964/0.33961 | 0.29098/0.17347 | — | 0.38580/0.27313 | 0.29031/0.17484 |
| 1234 / 15991 | GDN | 0.68700/0.34168 | 0.28657/0.17081 | — | — | — |
| 1234 / 15991 | QGDN Recall→Delta | 0.65581/0.36732 | 0.29243/0.17166 | 0.38914/0.29371/4.802% | — | — |
| 1234 / 15991 | Q-Delta 正号 | 0.68835/0.33940 | 0.28924/0.16910 | — | 0.37443/0.25989 | 0.28843/0.17150 |

剩余 QGDN `38987` 为 RUNNING，已记录 step `16601`，checkpoint 为
step `16000`；最近 20 点吞吐中位 `314.6k token/s`，峰值显存
`77.02 GB/GPU`。所有已记录 loss、梯度、吞吐、显存及门控数值有限，两条 Q-Delta
全历史收缩违规率为 0。六路冻结 commit、干净 worktree、实际科学配置、入口门禁、
各 seed 三路初始化 hash 和物理 T 关闭状态再次核验通过，无重复作业；checkpoint
目录/step 元数据可读取。

本轮六路 stderr 字节数、mtime 与 launcher 错误计数均未变化，仍为
`97/134/67/96/134/132`；启动时 NET/IB 告警未增长，旧诊断根因仍未确认，继续保留。
07:13 已回收 38987 的新日志和小型指标，仅缺最终 summary。五条终态作业
38983/38984/38985/38986/38988 已核验并回收；没有重启、重提或改环境。现有半小时
监控继续 ACTIVE，等待 seed 1234 的共同 step 18000 和完整 step 19073 终态。

## 三 seed 10BT 最终结论与监控结束（2026-09-06 08:23 CST）

seed 1234 的 QGDN Recall→Delta（Slurm `38987`）于 2026-09-06 08:22:01 CST
完成，最终 validation loss `2.699259297`、PPL `14.86871434`。本批六条全部完成，
加上 seed 3407 的三条历史对照，共九条均跑满 19073 step / 9,999,745,024 prediction tokens。
六条主作业及全部 Slurm 子任务均 COMPLETED/0:0，run.exitcode=0；08:24 已完成最后
一条的回收。九条本地 collection 均无缺失必需产物，summary、metrics 和 stdout 的
全部 validation 节点一致，训练日志与远端审计一致。最终 checkpoint ZIP 目录及
step=19073 元数据可读，model_final.pt 存在且无临时 checkpoint；未校验完整 tensor
payload CRC，大权重保留远端，日志及小型结果保存在本地 experiments 目录。

三种方法逐 seed 的共享初始化 hash 一致，九条使用同一数据 manifest、固定验证 seed
172903、相同 1600 条验证序列（6,553,600 scored tokens），8×H800、T4096、micro
batch 8、global batch 128、梯度累积 2、fused loss、关闭 activation checkpointing。
优化器、学习率日程、数值策略及日志/验证周期逐项一致；每种方法跨 seed 的模型配置一致。
GDN/QGDN 冻结于 `7eb73ca89411c54d4fe7a8ffb427df44e7709cfa`，Q-Delta 冻结于
`18dc085bb42d83f760ac7d5136f14b7ae75fcb15`，所有 worktree 干净。三组入口门禁均为
GDN 6/6、QGDN 6/6、Q-Delta 3/3。QGDN 为 beta-style gamma Recall→Delta，虚拟 2T
路径保持不变，`QGDN_USE_PHYSICAL_T=False`；Q-Delta 为论文正号。

| seed | 方法 | Slurm | 终点 validation loss | 终点 PPL | 同步20点训练 loss 均值 |
|---|---|---|---|---|---|
| 3407 | GDN | 37118 | 2.697353622 | 14.84040638 | 2.676577 |
| 3407 | QGDN Recall→Delta | 37183 | 2.696374571 | 14.82588399 | 2.675288 |
| 3407 | Q-Delta 正号 | 38035 | 2.695107604 | 14.80711198 | 2.674814 |
| 42 | GDN | 38983 | 2.695053703 | 14.80631387 | 2.684289 |
| 42 | QGDN Recall→Delta | 38984 | 2.695462307 | 14.81236503 | 2.684074 |
| 42 | Q-Delta 正号 | 38985 | 2.694630432 | 14.80004812 | 2.683803 |
| 1234 | GDN | 38986 | 2.699135894 | 14.86687961 | 2.676160 |
| 1234 | QGDN Recall→Delta | 38987 | 2.699259297 | 14.86871434 | 2.676153 |
| 1234 | Q-Delta 正号 | 38988 | 2.696995281 | 14.83508941 | 2.674711 |

终点同步训练窗口对三路严格取相同 20 个日志点：18891、18901、…、19071，加最终
19073。初始 validation 只用于入口检查；跨 seed 统计全部使用完整终点，未混用训练预算。

| 方法 | validation loss 均值 ± 样本SD | PPL 均值 ± 样本SD |
|---|---|---|
| GDN | 2.697181 ± 0.002047 | 14.837867 ± 0.030363 |
| QGDN Recall→Delta | 2.697032 ± 0.001982 | 14.835654 ± 0.029418 |
| Q-Delta 正号 | 2.695578 ± 0.001251 | 14.814083 ± 0.018532 |

| seed | Q-Delta 相对基线 | Δvalidation loss | ΔPPL | 相对 ΔPPL |
|---|---|---|---|---|
| 3407 | GDN | -0.002246017 | -0.03329441 | -0.22435% |
| 3407 | QGDN Recall→Delta | -0.001266967 | -0.01877201 | -0.12662% |
| 42 | GDN | -0.000423270 | -0.00626575 | -0.04232% |
| 42 | QGDN Recall→Delta | -0.000831875 | -0.01231691 | -0.08315% |
| 1234 | GDN | -0.002140613 | -0.03179019 | -0.21383% |
| 1234 | QGDN Recall→Delta | -0.002264016 | -0.03362492 | -0.22615% |

| 基线 | Δloss 均值 ± 样本SD | ΔPPL 均值 ± 样本SD | 相对 ΔPPL 均值 ± 样本SD（%） | 胜出 |
|---|---|---|---|---|
| GDN | -0.001603 ± 0.001023 | -0.023783 ± 0.015189 | -0.16017 ± 0.10220 | 3/3 |
| QGDN Recall→Delta | -0.001454 ± 0.000734 | -0.021571 ± 0.010926 | -0.14530 ± 0.07331 | 3/3 |

所有统计以 seed 为独立重复，`n=3`、样本标准差分母为 `n−1`；PPL 按每条完整终点
的 PPL 做算术平均，不将平均 loss 取指数冒充平均 PPL。配对差定义为 Q-Delta 减基线，
相对差为 `100 × (PPL_QDelta / PPL_baseline − 1)`，负值表示 Q-Delta 更好；相对差的
标准差单位为百分点。两个基线比较均 3/3 胜出（本组样本胜率 100%）。

Q-Delta 在三组 seed 上的终点收益方向一致，但幅度很小：相对 GDN 的平均配对 PPL
降低 0.16017%，相对 Recall→Delta 降低 0.14530%。seed 42 相对 GDN 仅低 0.04232%。
这些是三 seed 的描述性结果，不据此宣称统计显著或在其他数据、规模上普遍优越。
Recall→Delta 相对 GDN 只在 seed 3407 胜出，seed 42/1234 略差；其总体 PPL 均值接近。

本轮同时补齐 seed 1234 的共同 step 18000（9,437,184,000 prediction tokens），该节点
的同步训练窗口为 17801–17991 的相同 20 个日志点；作为学习曲线记录，不混入终点统计。

| seed 1234 / step 18000 方法 | validation loss | PPL | 同步20点训练 loss 均值 |
|---|---|---|---|
| GDN | 2.703193791 | 14.92733045 | 2.678761 |
| QGDN Recall→Delta | 2.703346970 | 14.92961718 | 2.679098 |
| Q-Delta 正号 | 2.701070595 | 14.89567042 | 2.677081 |

| seed / step 19073 | 方法 | alpha mean/std | beta mean/std | gamma mean/std/饱和率 | lambda mean/std | alignment mean/std |
|---|---|---|---|---|---|---|
| 3407 | GDN | 0.68316/0.34822 | 0.28844/0.16865 | — | — | — |
| 3407 | QGDN Recall→Delta | 0.65101/0.37879 | 0.29414/0.17227 | 0.39617/0.30182/4.761% | — | — |
| 3407 | Q-Delta 正号 | 0.68404/0.34831 | 0.28960/0.16919 | — | 0.37806/0.26132 | 0.28977/0.17193 |
| 42 | GDN | 0.69830/0.33863 | 0.28812/0.17109 | — | — | — |
| 42 | QGDN Recall→Delta | 0.66905/0.36843 | 0.28928/0.17202 | 0.39247/0.30419/5.357% | — | — |
| 42 | Q-Delta 正号 | 0.69964/0.33961 | 0.29098/0.17347 | — | 0.38580/0.27313 | 0.29031/0.17484 |
| 1234 | GDN | 0.68413/0.34358 | 0.28533/0.17086 | — | — | — |
| 1234 | QGDN Recall→Delta | 0.65413/0.36834 | 0.29078/0.17161 | 0.38644/0.29462/4.840% | — | — |
| 1234 | Q-Delta 正号 | 0.68582/0.34089 | 0.28762/0.16886 | — | 0.37618/0.26108 | 0.28680/0.17121 |

九条全部已记录 loss、grad_norm、吞吐、显存与方法特有门控值有限；三条 Q-Delta
全历史收缩违规率为 0。最终 QGDN gamma 饱和率在 4.761%–5.357%，未出现训练 OOM
或非有限指标。beta 饱和率未由冻结 logger 输出，不推测缺失统计。

本批六路保留的 `srun: error:` 计数为 `97/134/67/96/134/132`，stderr 字节数、mtime
及诊断内容均未较上一轮增长。历史 seed 3407 QGDN 有 118 条同类 launcher 诊断，
GDN/Q-Delta 无此类记录；九条均有 16 条启动 NET/IB 设备合并告警。结合完整终态证据
记录为保留的 launcher/启动告警，根因未确认，不能称为日志无错误或已修复。38983
结束阶段曾等待 launcher I/O 60 秒，该开销包含于 Slurm 分配时间。全程没有恢复、
重复提交或改动环境/科学配置，九条队列均已消失并由成功终态证据确认资源释放。

| 方法 | 摘要 GPU·h 均值 ± SD | 分配 GPU·h 均值 ± SD | 峰值 GB/GPU 均值 ± SD | 终点20点吞吐中位的跨 seed 均值（token/s） |
|---|---|---|---|---|
| GDN | 26.1691 ± 0.0703 | 26.9156 ± 0.2316 | 56.8190 ± 0.0043 | 864,801 |
| QGDN Recall→Delta | 71.4232 ± 0.6304 | 72.1081 ± 0.8397 | 77.0212 ± 0.0033 | 313,157 |
| Q-Delta 正号 | 45.7318 ± 0.2283 | 46.4830 ± 0.5429 | 66.2714 ± 0.0043 | 472,594 |

摘要 GPU·h 定义为 `8 × summary.wall_seconds / 3600`；Slurm 分配 GPU·h 使用实际
Start–End，额外包括入口门禁、启动和退出开销。train seconds 是训练入口累计的训练
计时，终点吞吐取各条最后 20 个 train 日志点的中位数，两种口径不混用。

三 seed 的 Q-Delta/基线配对摘要成本比均值 ± 样本SD：相对 GDN 为
`1.74756 ± 0.01160`，相对 Recall→Delta 为 `0.64034 ± 0.00878`。即相对 GDN 平均
多用 74.76% GPU·h，相对虚拟 2T QGDN 平均节省 35.97%；平均峰值显存分别约
56.82 / 77.02 / 66.27 GB/GPU（GDN / QGDN / Q-Delta）。Q-Delta 在本设置下比
Recall→Delta 更省且终点 PPL 更低，但相对 GDN 的小幅 PPL 收益需要付出明显成本。
各作业分布于不同节点和时间窗口，不能将上述全程成本比当作同机纯 kernel 加速比。

本地可复核统计：`preparation/qgdn-speed/audits/20260906-final-three-seed-report.json`；
本批终态审计 `20260906-082318-multiseed-reviewed.json`，历史复核
`20260906-082511-historical3407.json`，均位于同一 audits 目录。统计文件保存逐作业
experiment ID、summary SHA-256、全 Slurm 子任务终态、回收记录、同步窗口与完整精度结果。
远端原始结果位于 `/work/projects/memos-b3/code/wangzr/experiments/<experiment-id>/`。

结束条件已满足，现有 `qgdn-seed` 监控已暂停（PAUSED），没有创建替代监控；物理 T
继续按用户要求暂存。保留全部实验 worktree、checkpoint 和结果。

## 当前下一步

1. 本次六条复现、完整回收、三 seed 统计和文档归档已完成，现有监控已暂停，无待跑作业。
2. 保留冻结代码、数据、日志、checkpoint 和原始结果；后续实验范围等待用户新指示，不自动扩展或重跑。
3. 物理 T 继续暂存，不恢复优化，不启用未通过生产验证的路径。
