...
> seed 42/1234 六条完整 10BT 复现已完成；三 seed 最终统计与成本见文末，现有监控已暂停。
...
step-12000/14000 的第六、七个共同 validation 中，正号/负号 PPL 为
`15.80459/15.82089` 与 `15.37581/15.39630`，负号分别高 `0.1031%/0.1333%`。
正号相对 GDN 低 `0.1957%/0.2220%`，相对 beta-style Recall→Delta 低
`0.1002%/0.1202%`；负号相对 Recall→Delta 则高 `0.0028%/0.0129%`，已可视为
基本持平。对齐 step 15381 的近 20 点训练 loss 为 `2.68545/2.68631`，吞吐为
`467.7k/474.8k token/s`；节点不同，因此只把 loss/PPL 当作严格科学比较，吞吐仅作
健康检查。最新正号/负号到 step `17411/15421`，峰值显存为
`66.2665/66.2684 GB/GPU`，所有训练数值有限、收缩违规率为 `0`，日志和 checkpoint
持续更新。正号已有 step-16000 PPL `15.06649`，等待负号同节点结果。七个共同节点均由
正号领先，当前证据支持保留论文正号，但优势仍是约千分之一，必须以完整 10BT 终点确认。

两条完整 10BT 现均成功结束并回收。正号/负号 Slurm `38035/38065` 均为
`COMPLETED/0:0`、`run.exitcode=0`，最终 validation loss/PPL 为
`2.695108/14.80711` 与 `2.696814/14.83240`，正号领先 `0.1708%` PPL。正号最终
相对 GDN、beta-style Recall→Delta、beta-style Parallel 低
`0.2243%/0.1266%/0.1470%` PPL；负号相对 GDN 低 `0.0539%`，但相对 Recall→Delta
和 Parallel 高 `0.0440%/0.0236%`。step-16000/18000 也由正号领先
`0.1540%/0.1743%`，所以所有九个共同周期节点和终点方向一致。

末 20 点训练 loss 为 `2.674814/2.674842`，训练/墙钟有效吞吐为
`497.6k/488.7k` 与 `497.4k/486.9k token/s`，训练/墙钟时长为
`5.582/5.684 h` 与 `5.585/5.705 h`，峰值显存 `66.2665/66.2684 GB/GPU`。
末步 beta mean/std 为 `0.28960/0.16919` 与 `0.29348/0.17255`，lambda mean/std
...
## 六路启动完成与首个共同验证（2026-09-06 00:00 CST）

六条均 RUNNING，两个 seed 的三路入口门禁分别为 6/6、6/6、3/3，且各 seed 内共享初始化
hash 全部相同。冻结代码、科学配置和物理 T 关闭状态保持不变。当前吞吐使用近 20 个日志
样本的中位数，各方法运行在不同节点，以下只记录成本与健康状态。
...
seed 42 的共同 step-2000 validation loss/PPL 为 GDN `3.139244860/23.08642680`、
QGDN `3.140767889/23.12161488`、Q-Delta `3.140823772/23.12290702`。
Q-Delta 比 GDN 高 0.1580% PPL，与 QGDN 基本持平（高 0.0056%）。共同训练窗口
step 1801–1991 的 20 个日志点 loss 均值依次为 `3.136269/3.137020/3.137250`。
本 seed 首个节点未复现 seed 3407 的领先方向，不能提前确认跨 seed 收益；继续跑满。

五个已发布 checkpoint 的目录和元数据可读取；seed 1234 Q-Delta 尚未到首个保存点。
所有已记录训练指标有限，两条 Q-Delta 收缩违规率全为 0。00:01 已回收六路日志和小型指标，
最终 summary 尚缺。一次本机 SSH 转发端口冲突已由禁用该只读连接转发恢复，没有重启训练。
现有半小时监控继续，下一次效果判断等待 seed 42 共同 step 4000 和 seed 1234 共同 step 2000。

## 两个 seed 的共同验证与成本（2026-09-06 00:37 CST）

seed 42 完成共同 step 4000（2,097,152,000 prediction tokens），seed 1234 完成共同
step 2000（1,048,576,000 prediction tokens）。各节点均验证 1600 条序列、6,553,600 scored
tokens；训练窗口分别为 step 3801–3991、1801–1991，均取相同的最后 20 个日志点。
...
Q-Delta 相对 GDN / QGDN 的 PPL 差分别为 seed 42 `+0.0115% / -0.0444%`、
seed 1234 `-0.0017% / -0.1889%`。两节点均与 GDN 基本持平，略优于 QGDN；不同 seed
当前预算不同，不混合统计或提前认定稳定收益。
...
吞吐取各作业最近 20 个日志点中位数，各方法运行于不同节点。所有日志数值有限、两条
Q-Delta 收缩违规率仍为 0；六路入口、冻结配置和共享初始化均通过核验。六个 checkpoint
ZIP 目录及 step 元数据可读取，未进行完整 tensor payload CRC 校验。00:38 已回收日志和
小型指标，仍缺最终 summary；没有重启训练或重复提交。后续继续等待 seed 42 共同 step
6000 和 seed 1234 共同 step 4000，再于全部终态后汇总三个 seed 的效果、成本和稳定性。

## seed 42 共同 step 6000 与运行状态（2026-09-06 01:20 CST）

seed 42 三路完成共同 step 6000 / 3,145,728,000 prediction tokens，固定验证 1600 条
序列、6,553,600 scored tokens；训练比较取 step 5801–5991 的相同 20 个日志点。
...
Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.06035% / 0.04051%`，仍是小幅阶段收益。
seed 1234 尚待共同 step 4000，不跨不同预算混合统计。
...
吞吐为各自最近 20 个日志点中位数，节点和当前进度不同，仅用于成本/健康跟踪。五条
stderr 新增 Slurm 连接重置、消息长度/协议解析错误；01:18→01:20 复核错误计数未增长，
六路训练继续推进且所有指标有限，Q-Delta 收缩违规率全为 0。来源和根因尚未确认，
未重启、未修改环境；后续持续检查 launcher 诊断与终态退出码，详见 CURRENT_PROGRESS。

六路仍 RUNNING，冻结配置、入口和初始化 hash 核验通过；checkpoint 目录及元数据可读取。
01:19 已回收日志/小型指标，最终 summary 尚缺，大权重保留远端。监控继续等待 seed 42
共同 step 8000、seed 1234 共同 step 4000 和两条 GDN 的最终结果；物理 T 保持关闭。

## GDN 终态成本与 seed 1234 共同 step 4000（2026-09-06 01:55 CST）

两条 GDN 已跑满 19073 step / 9,999,745,024 prediction tokens，Slurm 主作业及子 step
均 COMPLETED/0:0，run.exitcode=0，最终 metrics/summary/stdout 一致，终点 checkpoint
元数据和 model_final.pt 已核验。01:56 终态日志与小型结果回收成功，大权重保留远端。
...
摘要 GPU·h 按 `8 × summary.wall_seconds / 3600`，两路 wall_seconds 分别为
`11792.782931/11739.613108`；Slurm 分配成本另含入口/启动和退出耗时。38983 的 launcher
在结束阶段等待 I/O 60 秒，最终验证完整、退出为 0，保留该记录计入稳定性评估。

seed 1234 的三路共同 step 4000 验证使用相同 1600 条序列、6,553,600 scored tokens；
训练均取 step 3801–3991 的最后 20 个共同日志点。
...
Q-Delta 相对 GDN / QGDN 的 PPL 分别为 `+0.12522% / -0.04299%`。相对 GDN 未见一致
领先方向，仍待最终同预算多 seed 统计。seed 42 最新共同节点仍为 step 6000。
...
吞吐按最近 20 个日志点中位数，各方法位于不同节点。Slurm 协议/连接诊断继续增加，
目前六路均有记录；两条 GDN 成功完成，四路仍持续输出有限指标，Q-Delta 收缩违规率全为 0。
根因未确认，未重启或改环境。01:56 四条运行中作业的日志/小型指标已回收，仅缺最终
summary。监控继续等待 seed 42 共同 step 8000、seed 1234 共同 step 6000 和其余四路终态；
物理 T 保持关闭。

## 两个 seed 新共同节点与成本（2026-09-06 02:31 CST）

seed 42 / 1234 的共同节点分别为 step 8000 / 6000，训练预算分别为
4,194,304,000 / 3,145,728,000 prediction tokens；各验证 1600 条序列、6,553,600 scored
tokens。同步训练窗口分别取 step 7801–7991 / 5801–5991 的相同 20 个日志点。
...
Q-Delta 相对 GDN / QGDN 的 PPL 改善分别为 seed 42 `0.06774% / 0.09142%`、
seed 1234 `0.05695% / 0.12837%`。两节点均小幅领先，仍待最终同预算多 seed 结论。
...
两条 GDN 的终态结果/成本不变，其余四条均 RUNNING，数值有限、Q-Delta 收缩违规率为 0。
上表吞吐取最近 20 个日志点中位数，各方法位于不同节点。38985 新增九条既有类型 Slurm
消息解析诊断，最后记录为 02:01:40；其他错误计数未增长，根因仍未确认，未重启或改环境。
02:32 已回收运行中日志和小型指标，仅缺最终 summary。冻结配置、门禁、初始化 hash、
checkpoint 目录/元数据均再次核验，物理 T 关闭；继续等待共同 step 10000 / 8000。

## seed 42 共同 step 10000 与成本（2026-09-06 03:06 CST）

共同 step 10000 为 5,242,880,000 prediction tokens，固定验证 1600 条序列、6,553,600
scored tokens；三路训练窗口均取 step 9801–9991 的最后 20 个相同日志点。
...
Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.03917% / 0.05379%`，收益仍小且较上节点
缩小，等待最终同预算多 seed 判断。seed 1234 尚待共同 step 8000。
...
四路继续训练、数值有限、Q-Delta 收缩违规率为 0；两条 GDN 已完成的终态成本不变。
上表吞吐为各自最近 20 个日志点中位数，节点/进度不同。本轮六路 launcher 错误计数及
stderr 均未变化，旧诊断继续保留。03:07 已回收四路日志/小型指标，仅缺最终 summary。
冻结配置、门禁、初始化和 checkpoint 目录/元数据均核验，物理 T 关闭，未重启或改环境；
下一次等待 seed 42 / 1234 的共同 step 12000 / 8000。

## seed 1234 共同 step 8000 与成本（2026-09-06 03:40 CST）

共同 step 8000 为 4,194,304,000 prediction tokens，固定验证 1600 条序列、6,553,600
scored tokens；三路训练窗口同取 step 7801–7991 的 20 个日志点。
...
Q-Delta 相对 GDN / QGDN 的 PPL 分别低 `0.10189% / 0.17038%`，较上节点优势扩大，
仍等待最终同预算多 seed 统计。seed 42 最新三路共同验证仍为 step 10000。
...
四路训练持续推进、数值有限、Q-Delta 收缩违规率为 0；两条 GDN 的终态成本不变。
吞吐取近期 20 点中位数，各作业节点/进度不同。seed 42 Q-Delta 到 step 17861，尚未完成。
本轮六路 launcher 错误计数与 stderr 均未增长，旧诊断继续保留。03:41 已回收新日志与
小型指标，仍缺最终 summary；冻结配置、门禁、初始化和 checkpoint 目录/元数据再次
核验，物理 T 关闭，未重启或改环境。后续等待共同 step 12000 / 10000 和 Q-Delta 终态。

## seed 42 Q-Delta 终态成本与两个新共同节点（2026-09-06 04:14 CST）
...
| seed | Slurm | final validation loss | final PPL | train seconds | wall seconds | 摘要 GPU·h | Slurm 分配 GPU·h | 峰值 GB/GPU |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 42 | 38985 | 2.694630432 | 14.80004812 | 20219.68 | 20649.74 | 45.8883 | 46.8356 | 66.27 |

摘要 GPU·h 按 `8 × summary.wall_seconds / 3600`；Slurm 分配时间为 05:51:16，另含入口、
启动和退出耗时。最终训练门控 alpha/beta mean/std 为 `0.69964/0.33961`、
`0.29098/0.17347`，lambda 为 `0.38580/0.27313`，alignment 为 `0.29031/0.17484`，
全历史收缩违规率为 0。两条已完成 GDN 的终点与成本不变。
...
## seed 42 共同 step 14000 与成本（2026-09-06 04:53 CST）
...
## seed 1234 Q-Delta 终态成本与共同 step 12000（2026-09-06 05:30 CST）
...
| seed | Slurm | final validation loss | final PPL | train seconds | wall seconds | 摘要 GPU·h | Slurm 分配 GPU·h | 峰值 GB/GPU |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1234 | 38988 | 2.696995281 | 14.83508941 | 20190.40 | 20626.74 | 45.8372 | 46.7556 | 66.27 |

摘要 GPU·h 按 `8 × summary.wall_seconds / 3600`；Slurm 分配时间为 05:50:40，另含入口、
启动和退出耗时。最终训练门控 alpha/beta mean/std 为 `0.68582/0.34089`、
`0.28762/0.16886`，lambda 为 `0.37618/0.26108`，alignment 为 `0.28680/0.17121`，
全历史收缩违规率为 0。另三条已完成作业的终点与成本保持不变。
...
## 两个 seed 的新共同节点与成本（2026-09-06 06:04 CST）
...
## seed 42 共同 step 18000 与成本（2026-09-06 06:38 CST）
...
## seed 42 三路终态成本与 seed 1234 共同 step 16000（2026-09-06 07:12 CST）
...
| seed 42 方法 | Slurm | train seconds | wall seconds | 摘要 GPU·h | 峰值 GB/GPU |
|---|---:|---:|---:|---:|---:|
| GDN | 38983 | 11467.15 | 11792.78 | 26.2062 | 56.81 |
| QGDN Recall→Delta | 38984 | 31519.11 | 31983.60 | 71.0747 | 77.02 |
| Q-Delta 正号 | 38985 | 20219.68 | 20649.74 | 45.8883 | 66.27 |

摘要 GPU·h 按 `8 × summary.wall_seconds / 3600`。新完成 QGDN 的 Slurm 分配时间为
08:57:09，分配成本 `71.6200 GPU·h`，另含入口、启动和退出耗时。seed 42 Q-Delta 的
摘要成本为 GDN 的 `1.7510x`、QGDN 的 `0.6456x`；各作业运行于不同节点，成本与上述
小幅效果收益一并记录，最终仍以三 seed 整体结果判断。
...
## 三 seed 10BT 最终效果与成本（2026-09-06 08:23 CST）
...
| seed | 方法 | Slurm | train seconds | wall seconds | 摘要 GPU·h | 分配 GPU·h | 峰值 GB/GPU |
|---|---|---|---|---|---|---|---|
| 3407 | GDN | 37118 | 11427.39 | 11795.90 | 26.2131 | 27.1378 | 56.8215 |
| 3407 | QGDN Recall→Delta | 37183 | 32016.34 | 32467.94 | 72.1510 | 73.0778 | 77.0250 |
| 3407 | Q-Delta 正号 | 38035 | 20094.90 | 20461.43 | 45.4699 | 45.8578 | 66.2665 |
| 42 | GDN | 38983 | 11467.15 | 11792.78 | 26.2062 | 26.9333 | 56.8140 |
| 42 | QGDN Recall→Delta | 38984 | 31519.11 | 31983.60 | 71.0747 | 71.6200 | 77.0193 |
| 42 | Q-Delta 正号 | 38985 | 20219.68 | 20649.74 | 45.8883 | 46.8356 | 66.2739 |
| 1234 | GDN | 38986 | 11411.30 | 11739.61 | 26.0880 | 26.6756 | 56.8215 |
| 1234 | QGDN Recall→Delta | 38987 | 31533.43 | 31969.84 | 71.0441 | 71.6267 | 77.0193 |
| 1234 | Q-Delta 正号 | 38988 | 20190.40 | 20626.74 | 45.8372 | 46.7556 | 66.2739 |
