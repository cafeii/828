...
## 共享归一化与相邻视图调度诊断终态审查（job 34947）

- Slurm `COMPLETED`，exit code `0:0`，elapsed `00:03:42`；`run.exitcode=0`。`run.json`、完整日志和必需的 `outputs/result.json` 均已核对并回收。
- FP32 候选相对当前路径：输出最大绝对差 `2.4849e-4`、终态 `1.9297e-5`、完整梯度 `1.0226e-3`；差异来自等价递推的 chunk 边界重分组。QR 关闭时原生 GDN 输出逐位一致，BF16 前向和反向均有限。
- 同卡 340M、sequence 4096、micro batch 1：GDN `18,281.76` token/s，当前 QR-GDN `12,814.70` token/s，候选 `13,163.95` token/s。候选相对当前快 `2.73%`，但 QR-GDN/GDN 仅 `72.01%`，仍低于 `80%` 门槛。
- 峰值显存 GDN/当前 QR/候选 QR 为 `6.6799/6.7180/6.6960 GB`，没有异常增长。
- 结论：此候选只保留为调度证据，不改生产路径、不提交 8 卡 DDP 或正式训练。现有局部融合与调度清理后的最好证据仍不足，下一步必须重新设计能够共享 KV/QR 状态准备与扫描调度的专用耦合双状态 kernel，同时保持方法语义和 80% 门槛。
